...
if length(size(image_stack)) == 4
    ...
    normal = 0;
    for c = 1:channel
        image_stack_channel = squeeze(image_stack(:,:,c,:));
        [alb_ch, norm_ch, res] = estimate_alb_nrm(image_stack_channel, scriptV);
        if albedo == 0
            albedo = alb_ch;
        else
            albedo = cat(3, albedo, alb_ch);
        end
        disp('Normal channel size')
        disp(size(norm_ch));
        if normal == 0
            normal = norm_ch;
        else 
            normal = normal + norm_ch;
        end
    end
    normal = normal/3;
else
    ...
    if shadow_trick == true;
        for r = 1:h;
            for c = 1:w;
                 i = squeeze(image_stack(r,c,image_indices));
                 scriptI = diag(i);
                 g= mldivide(scriptI*scriptV, scriptI*i);
                 residual = scriptI*scriptV*g - scriptI*i;
